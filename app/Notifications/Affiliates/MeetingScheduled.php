...
class MeetingScheduled extends Notification
{
    ...
    {
        $firstLine = 'In response to your application to become a ' . config('app.name') . ' affiliate,';
        $firstLine .= 'you have been invited to a meeting with the management of the company.';
        return (new MailMessage)
                    ->subject(config('app.name') . ": Meeting Scheduled")
                    ->greeting('Hello')
                    ->line($firstLine)
                    ->line('See below the details of this meeting:')
                    ...
                    ->action('Go to Dashboard', route('affiliates.dashboard'))
                    ->line('Thank you!');
    }
