...
class BidRejectedNotification extends Notification
{
    ...
    {
        $mailMessage = (new MailMessage)->subject(config('app.name') . ': Bid Rejected');
        
        if($notifiable->id === $this->bidder->id) {
            $message = $mailMessage->line('Hello, your bid on ' . $this->loanFund->reference . ' has been rejected')
                    ->action('View your bids', $this->bidLink)
                    ->line('Thank you for using ' . config('app.name'));
        } else {
            $message = $mailMessage->line('Hello, you just rejected a bid on ' . $this->loanFund->reference)
    ...
    {
        if($notifiable->id === $this->bidder->id) {
            $message = 'Hello, your bid on ' . $this->loanFund->reference . ' has been rejected';
            ...
            $message = 'Hello, you just rejected a bid on ' . $this->loanFund->reference;
