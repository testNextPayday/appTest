...
class BidUpdatedNotification extends Notification
{
    ...
    public function toMail($notifiable)
    {
        ...
        $mailMessage = (new MailMessage)->subject(config('app.name') . ': Bid Updated');
        
        if($notifiable->id === $this->bidder->id) {
            $message = $mailMessage->line('Hello, you just updated your bid on ' . $this->loanFund->reference . '.')
                    ->line('New Amount\t₦' . $this->bid->amount)
                    ->action('View your bids', $this->bidLink)
                    ->line('Thank you for using ' . config('app.name'));
        } else {
            $message = $mailMessage->line('Hello, a bid on, ' . $this->loanFund->reference . ' has just been updated.')
                    ->line('New Amount\t₦' . $this->bid->amount)
    ...
    {
        if($notifiable->id === $this->bidder->id) {
            $message = 'Hello, you just updated your bid on loan, ' . $this->loanFund->reference . '.' .PHP_EOL;
            $message .= 'New Amount: ₦' . $this->bid->amount;
            ...
            $message = 'Hello, a bid on your loan, ' . $this->loanFund->reference . ', has just been updated.' .PHP_EOL;
            $message .= 'New Amount: ₦' . $this->bid->amount;
