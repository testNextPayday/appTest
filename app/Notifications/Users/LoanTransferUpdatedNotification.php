...
class LoanTransferUpdatedNotification extends Notification
{
    use Queueable;
    ...
    {
        $mailMessage = (new MailMessage)->subject(config('app.name') . ': Loan Transfer Updated');
        
        if($notifiable->id === $this->loanOwner->id) {
            $message = $mailMessage->line('Hello, you just updated your loan transfer request for ' . $this->loanFund->reference)
                    ->line('New Amount\t₦' . $this->loanFund->sale_amount)
                    ->action('View Loan', $this->loanLink)
                    ->line('Thank you for using ' . config('app.name'));
                    
        } else {
            $message = $mailMessage->line('Hello, the transfer terms for ' . $this->loanFund->reference . ' has just been updated.')
                    ->line('New Amount\t₦' . $this->loanFund->sale_amount)
    ...
    {
        if($notifiable->id === $this->loanOwner->id) {
            $message = 'Hello, you just updated your transfer terms for ' . $this->loanFund->reference.PHP_EOL;
            $message .= 'New Amount: ₦' . $this->loanFund->sale_amount;
            ...
            $message = 'Hello, the transfer terms for ' . $this->loanFund->reference . ' has just been updated'.PHP_EOL;
            $message .= 'New Amount: ₦' . $this->loanFund->sale_amount;
