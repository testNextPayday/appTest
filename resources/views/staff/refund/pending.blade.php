                    ...
                    <table id="order-listing" class="table table-responsive">
                        ...
                        @forelse($refunds as $index => $refund)
                            <tr>
                                ...
                                <td>
                                    <button class="btn btn-secondary btn-sm" data-toggle="modal" data-target="#refund" data-amount="{{$refund->amount}}" data-user_id="{{$refund->loanInfo->user_id}}" data-reason="{{$refund->reason}}" data-loan_id="{{$refund->loanInfo->id}}" data-name="{{$refund->getUSerInfo->name}}" data-reference="{{$refund->loanInfo->reference}}" data-url_data="{{route('staff.refund.update', ['id'=>$refund->id, 'status'=>0])}}">Update</button>

                                    <form style="display:inline" method="POST" action="{{route('staff.refund.update', ['id' => $refund->id, 'status' => 2])}}">
                                		{{ csrf_field() }} {{ method_field('PATCH') }}
                                		<button class="btn btn-danger btn-sm" type="submit">Reject</button>
                                	</form>

                                    <form style="display:inline" method="POST" action="{{route('staff.refund.update', ['id' => $refund->id, 'status' => 1])}}">
                                		{{ csrf_field() }} {{ method_field('PATCH') }}
