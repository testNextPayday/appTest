          ...
          <td>
            <div>
              ...
              <span style="float:right"><span style="text-decoration:line-through">{{number_format($loan->balance,2)}}</span> &nbsp;
                <span>{{number_format($loan->repaymentPlans->last()->getBalanceColumnDebit(),2)}}</span></span>
              @else
    ...
    <table>
      ...
      <tbody>
        ...
        <tr class="repayments">
          ...
          <td>
            @if($plan->status == 0)
            {{number_format($plan->getBalanceColumnDebit(),2)}}
            @endif
          </td>
        </tr>
        @if($plan->status == 1)
        ...
        @if($plan->transactions->count() > 0)
          ...
          @php
          $wallet_balance = optional($tranx)->wallet_balance > 0 ? - optional($tranx)->wallet_balance : abs(optional($tranx)->wallet_balance);

          @endphp
        <tr>
          <td>{{optional($tranx)->created_at->toDateString()}}</td>
          <td class="tranx">{{optional($tranx)->name}}</td>
          <td>{{optional($tranx)->type == '1' ? number_format(optional($tranx)->amount,2) : ''}}</td>
          <td>{{optional($tranx)->type == '2' ? number_format(optional($tranx)->amount,2) : ''}}</td>
        ...
        <tr style="font-weight:bold;">
          ...
          <td>
            ...
            <span>{{number_format($loan->repaymentPlans->last()->getBalanceColumnDebit(),2)}}</span>
            @else
