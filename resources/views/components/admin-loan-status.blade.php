...
@switch($loan->status)
    ...
    @case(1)       
        ...
        @if(request()->user('admin'))           
            <form action="{{route('admin.loan.fulfill',['reference'=>$loan->reference])}}" method="POST" style="display:inline">
                @csrf
                <button class="btn btn-success">Fulfill</button>
            </form>
    ...
    @case(2)
        ...
        @if(request()->user('admin'))
            <form action="{{route('admin.loans.unfulfill',['reference'=>$loan->reference])}}" method="POST" style="display:inline">
                @csrf
                <button class="btn btn-success">Unfulfill</button>
            </form>
        @endif
    @break
