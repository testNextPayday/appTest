                    ...
                    <table class="table table-bordered">
                        <tbody>
                            ...
                            <tr>
                                        ...
                                        <td>{{$settlement->investors_cut ? number_format($settlement->investors_cut,2) : 'N/A'}}</td>
                                    </tr>
                            <tr>
                                <td>Date Paid</td>
                            ...
                            <tr>
                                <td>Loan</td>
                                <td>{{$settlement->loan->reference}} ({{number_format($settlement->loan->amount,2)}}) </td>
                            </tr>
                        </tbody>
                    </table>
                            ...
                            <div class="modal-dialog" role="document">
                                <div class="modal-content">
                                    ...
                                    <div class="modal-body">
                                        <form action="{{route('admin.settlement.decline',['reference'=>$settlement->reference])}}" method="POST">
                                            @csrf
    ...
    <div class="card">
        <div class="card-body">
            ...
            <div class="row">
                ...
                @endif
                @foreach($loan->repaymentPlans as $plan)
                <form method="post" id="confirm-repayment{{$plan->id}}" action="{{route('admin.repayment.confirm',$plan)}}">
                    {{ csrf_field() }}
                </form>
                @endforeach
                @foreach($loan->repaymentPlans as $plan)
                <form method="post" id="unconfirm-repayment{{$plan->id}}" action="{{route('admin.repayment.unconfirm',$plan)}}">
                    {{ csrf_field() }}
                </form>
                @endforeach
                @foreach($loan->repaymentPlans as $plan)
                <form method="post" id="delete-repayment{{$plan->id}}" action="{{route('admin.repayment.delete',$plan)}}">
                    {{ csrf_field() }}
                    {{method_field('DELETE')}}
                </form>
                @endforeach
