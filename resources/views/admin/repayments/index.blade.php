                        ...
                        <table id="order-listing" class="table">
                            ...
                            <tbody>
                                @forelse($plans as $plan)
                                    <tr>
                                        <td>{{ optional($plan->loan->user)->name }}</td>
                                        ...
                                        <td>
                                            ...
                                            @else
                                                @if($plan->payday <= Carbon\Carbon::today() && !$plan->status)
                                                    <a class="btn btn-xs btn-warning" href="{{route('admin.repayments.order_debit', ['plan_id' => encrypt($plan->id)])}}">Issue Debit Order</a>
                                                    <form action="{{route('admin.repayments.try-card',$plan)}}" method="POST">
                                                        {{csrf_field()}}
                                                        <button type="submit" class="btn btn-xs btn-warning">Sweep</button>
                                                    </form>
                                                @elseif($plan->payday <= Carbon\Carbon::today() && $plan->status)
                                                    <a class="badge badge-success" href="#">Paid</a>
                                                @else
