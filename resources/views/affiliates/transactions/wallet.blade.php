@extends('layouts.affiliates')

@section('content')

<div class="content-wrapper">
    <div class="row mb-4">
        <div class="col-12 d-flex align-items-center justify-content-between">
            <h4 class="page-title">Wallet Transactions</h4>
        </div>
    </div>
    
    <div class="row">
        <div class="col-md-12">
            <div class="card support-pane-card">
                <div class="card-body">
                    <div class="d-flex justify-content-between align-items-center mb-3">
                        <h4 class="card-title mb-0">Wallet Transactions</h4>
                    </div>
                    <div class="table-responsive support-pane no-wrap">
                        @if(count($transactions))
                        @foreach($transactions as $transaction)
                        <div class="t-row">
                            <div class="tumb">
                                <div class="img-sm rounded-circle bg-info d-flex align-items-center justify-content-center text-white">
                                    WT
                                </div>
                            </div>
                            <div class="content">
                                <p class="font-weight-bold mb-2 d-inline-block">
                                    {{ strtoupper($transaction->reference) }}
                                </p>
                                <p class="text-muted mb-2 d-inline-block">
                                    <small>&nbsp;REF</small> &nbsp;&nbsp;
                                    <small>
                                        @if($transaction->direction == 1)
                                            <i class="text-success icon-action-undo"></i>
                                        @else
                                            <i class="text-warning icon-action-redo"></i>
                                        @endif
                                    </small>
                                </p>
                                <p>
                                  {{$transaction->description}}
                                </p>
                            </div>
                            <div class="tile">
                                <p class="text-muted mb-2">Amount</p>
                                <p class="font-weight-bold">₦ {{ number_format($transaction->amount, 2) }}</p>
                            </div>
                            <div class="content">
                                <p class="text-muted mb-2">Date</p>
                                <p class="font-weight-bold">
                                    {{ $transaction->created_at->format('l jS \\of F Y') }}
                                </p>
                            </div>
                        </div>
                        @endforeach
                        @else
                        <div class="t-row text-center">
                            <h4>You've made no transactions yet</h4>
                        </div>
                        @endif
                    </div>
                    <div class="d-flex justify-content-between align-items-center mt-4">
                        <p class="mb-0 d-none d-md-block"></p>
                        <nav>
                          {{$transactions->links('layouts.pagination.default')}}
                        </nav>
                    </div>
                </div>
            </div>
        </div>
    </div>
</div>
<!-- content-wrapper ends -->
@endsection
