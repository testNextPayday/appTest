    ...
    <form method="POST" :action="url" class="card p-5">
		...
		<div class="row" v-if="loaded">
			...
			<div class="col">
				...
				<select type="text" class="form-control" name="investorId"   id="from" v-model="selectedInvestor">
					<option v-for="(investor,index) in investors" :value="investor.id" :key="index"  :data-index="index" > {{investor.name }} </option>
				</select>
			</div>

            <div class="col">
				<label for="from">Promissory Note</label>
				<select type="text" class="form-control" name="note_id"   id="from">
					<option v-for="(note, index) in pnotes" :value="note.id" :key="index">({{note.reference}}) {{note.principal }} </option>
				</select>
			</div>
...
export default {
    name : 'settle-affiliates-promissory-note',
    ...
    computed:{
        ...
        pnotes:  {
            set: function(newValue){
                this.currentInvestor.notes = newValue;
            },
            get : function() {
                return this.currentInvestor.notes;
            }
        }
    },
    ...
    watch : {
        ...
        selectedInvestor(value) {
            ...
            this.notes = this.currentInvestor.notes;
    ...
    methods : {
        ...
        async getAllNeededData() {
            await axios.get(`/ucnull/promissory-note/commission/data`).then((res)=> {
                let data = res.data;
