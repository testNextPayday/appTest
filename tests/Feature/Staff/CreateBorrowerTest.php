...
class CreateBorrowerTest extends TestCase
{
    ...
    public function testAffiliateCanCreateBorrower()
    {
        ...
        $route = route('staff.accounts.storeV2');
        Notification::fake();
        $response = $this->actingAs($staff, 'staff')->post($route, $data);
        ...
        $docUrl = route('staff.accounts.uploadDocs');

        $response = $this->actingAs($staff, 'staff')->post($docUrl, $documents);
